...
void runHeadless(std::string const &fileList, unsigned long nrEpisodes)
{
    ...
    agent->run();
    std::ofstream out{"results/rewardsSARSA.txt"};
    std::vector<float> const &rewards = agent->getRewards();
    copy(rewards.begin(), rewards.end(),
         std::ostream_iterator<float>(out, "\n"));
    std::ofstream opponent{"results/opponentPredictionLossesSARSA.txt"};
    ...
    std::ofstream opponentPerc{"results/opponentPredictionPercentageSARSA.txt"};
